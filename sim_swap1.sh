...
# RECONDUCT 9876543210 1234567890ABCDEF 1234567890 5555555555 1000
...
COOKIES=$(curl -sL -o /dev/null -w "%{http_cookie}\n" "$URL") || handle_error "Failed to extract cookies"
HEADERS=$(curl -sL -o /dev/null -w "%{header}\n" "$URL") || handle_error "Failed to extract headers"
...
CURL_COMMAND="curl -X POST http://192.168.8.1/api/ussd/send -d '{\"phoneNumber\":\"${PHONE_NUMBER}\",\"message\":\"${SMS_MESSAGE}\"}'"
...
curl_output=$(eval "$CURL_COMMAND") || handle_error "Failed to execute cURL command"

# Check the cURL output

if [[ "$curl_output" != "success" ]]; then
  handle_error "Failed to send SMS to 7147: $curl_output"
else
